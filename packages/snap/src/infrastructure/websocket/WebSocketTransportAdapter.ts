...
/**
 * WebSocket transport adapter for handling Solana RPC WebSocket connections.
 * Adapts the Snap WebSocket API to the WebSocketTransportPort interface.
 * Manages connection lifecycle, subscriptions, and message routing.
 */
export class WebSocketTransportAdapter implements WebSocketTransportPort {
  ...
  async openConnection(network: Network): Promise<string> {
    try {
      ...
      this.#logger.info(
        `[WebSocketTransportAdapter] Opening connection to ${wsUrl}`,
      );
      ...
      this.#logger.info(
        `[WebSocketTransportAdapter] Connected with ID: ${connectionId}`,
      );

      return connectionId;
    } catch (error) {
      this.#logger.error(
        '[WebSocketTransportAdapter] Failed to open connection:',
        error,
      );
      throw error;
    }
  ...
  async closeConnection(network: Network): Promise<void> {
    ...
    try {
      ...
      this.#logger.info(
        `[WebSocketTransportAdapter] Closed connection ${connectionId}`,
      );
    } catch (error) {
      this.#logger.error(
        '[WebSocketTransportAdapter] Failed to close connection:',
        error,
      );
    }
  }
  ...
  async unsubscribe(subscriptionId: string): Promise<void> {
    ...
    this.#logger.info(
      `[WebSocketTransportAdapter] Unsubscribed: ${subscriptionId}`,
    );
  }
    ...
    try {
      ...
      if (
        ...
        if (messageData.type === 'text') {
          ...
          this.#logger.warn(
            '[WebSocketTransportAdapter] Unknown message data type:',
            messageData.type,
          );
          return;
        }
      ...
      if (this.#isNotification(parsedMessage)) {
        ...
        this.#logger.error(
          '[WebSocketTransportAdapter] Received error:',
          parsedMessage.error,
        );
      }
    } catch (error) {
      this.#logger.error(
        '[WebSocketTransportAdapter] Failed to handle message:',
        error,
      );
    }
  }
  ...
  async handleConnectionEvent(
    ...
    this.#logger.info(
      `[WebSocketTransportAdapter] Connection event: ${event} for ${connectionId}`,
    );
  ...
  #handleNotification = async (
    ...
    if (!subscriptionId) {
      this.#logger.warn(
        `[WebSocketTransportAdapter] No subscription found for RPC ID: ${rpcSubscriptionId}`,
      );
      return;
    }
    ...
    if (!subscription) {
      this.#logger.warn(
        `[WebSocketTransportAdapter] No subscription found for ID: ${subscriptionId}`,
      );
      return;
    }
    ...
    try {
      ...
      this.#logger.error(
        `[WebSocketTransportAdapter] Error in subscription callback for ${subscriptionId}:`,
        error,
      );
    }
  };
  ...
  #handleSubscriptionConfirmation = (message: WebSocketMessage): void => {
    if (typeof message.result === 'number' && message.id) {
      ...
      this.#logger.info(
        `[WebSocketTransportAdapter] Subscription confirmed with RPC ID: ${message.result}`,
      );
    }
  };
  ...
  #handleDisconnection = async (connectionId: string): Promise<void> => {
    if (this.#reconnectAttempts < this.#maxReconnectAttempts) {
      ...
      this.#logger.info(
        `[WebSocketTransportAdapter] Attempting reconnection ${this.#reconnectAttempts}/${this.#maxReconnectAttempts} in ${delay}ms`,
      );
      ...
      setTimeout(() => {
        const reconnect = async (): Promise<void> => {
          try {
            ...
            this.#logger.error(
              '[WebSocketTransportAdapter] Reconnection failed:',
              error,
            );
          }
        };
      ...
      this.#logger.error(
        '[WebSocketTransportAdapter] Max reconnection attempts reached',
      );
    }
  };
  ...
  #recoverSubscriptions = async (connectionId: string): Promise<void> => {
    this.#logger.info(
      '[WebSocketTransportAdapter] Recovering subscriptions after reconnection',
    );
    ...
    for (const subscription of this.#subscriptions.values()) {
      try {
        ...
        this.#logger.error(
          `[WebSocketTransportAdapter] Failed to recover subscription ${subscription.id}:`,
          error,
        );
      }
    }
    ...
    for (const callback of this.#connectionRecoveryCallbacks) {
      try {
        ...
        this.#logger.error(
          '[WebSocketTransportAdapter] Error in connection recovery callback:',
          error,
        );
      }
    }
