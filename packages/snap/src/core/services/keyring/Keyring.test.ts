...
describe('SolanaKeyring', () => {
  ...
  describe('findExistingAccounts', () => {
    ...
    beforeEach(() => {
      ...
      // Creates a new keyring instance with our mocked AssetsService
      keyring = new SolanaKeyring({
        state: new SolanaState(),
        encryptedState: new EncryptedSolanaState(),
        configProvider: mockConfigProvider,
    ...
    it('finds accounts with non-zero balances', async () => {
      ...
      expect(existingAccounts[0]).toEqual({
        index: 0,
    ...
    it('returns empty array when no accounts have balance', async () => {
      ...
      expect(mockAssetsService.getNativeAsset).toHaveBeenCalled();
    });
