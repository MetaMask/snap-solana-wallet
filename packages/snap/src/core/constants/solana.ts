 ...
 */
export const NativeCaip19IdStruct = pattern(
  string(),
 ...
 */
export const TokenCaip19IdStruct = pattern(
  string(),
 ...
 */
export const NftCaip19IdStruct = pattern(
  string(),
  /^solana:[a-zA-Z0-9]+\/nft:[a-zA-Z0-9]+$/u,
);

/**
 * Custom struct that validates a string and returns it as TokenCaip19Id
 * This is useful when the API returns a generic string that needs to be validated
 * and typed as TokenCaip19Id
 */
export const TokenCaip19IdFromStringStruct = define(
  'TokenCaip19IdFromString',
  (value) => {
    if (typeof value !== 'string') {
      return `Expected a string, but received: ${typeof value}`;
    }

    const [error] = TokenCaip19IdStruct.validate(value);
