...
export function parseCardMessage(base64Message: string): {
  ...
  const regex =
    /^(.+) wants you to sign in with your Solana account:\s+([1-9A-HJ-NP-Za-km-z]{32,44})\s+([\s\S]+?)\s+URI:\s+(.+)\s+Version:\s+(\d+)\s+Chain ID:\s+(\d+)\s+Nonce:\s+([a-zA-Z0-9]+)\s+Issued At:\s+((?:(?!\s+Expiration Time:).)+)(?:\s+Expiration Time:\s+(.+))?$/u;
